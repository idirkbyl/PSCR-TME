...
int main () {
    ...
    unordered_map<string, int> map(1000);
	...
	while (input >> word) {
        ...
        std::pair<unordered_map<string, int>::iterator, bool> res = map.insert(std::make_pair(word, 1));
        if(!res.second){
            res.first->second++;
        }
   ...
   vector<string> words2 = {"war", "peace", "love", "hate", "death", "life","toto"};
    for(auto &word : words2){
        int occurences = map.find(word) != map.end() ? map.find(word)->second : 0;
        assert(count_if_equal(words.begin(), words.end(), word) == occurences);
    ...
    for(auto &entry : map){
        map2[entry.second].push_front(entry.first);
    }

    vector<int> numbers = {10,100,1000,298};
    for(auto &number : numbers){
        auto it = map2.find(number);
        if(it!=map2.end()){
            printf("These words appear %d times:\n", number);
